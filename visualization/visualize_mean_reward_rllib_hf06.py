...
"""
    ...
    Here, run on the system trained on uneven height fields (smoothness = 0.6)
    for three different architectures.
"""
...
fig.suptitle("Comparison height field (0.6) and flat training")
...
fig.suptitle("Return during training on height field (0.6) ")

for i in range(0, 3):
    # Use matplotlib's fill_between() call to create error bars.   
    plt.fill_between(time_steps, all_exp_data[i*2][2],  
                     all_exp_data[i*2][3], color=tableau20[i*2 + 1], alpha=0.25)  
    plt.plot(time_steps, all_exp_data[i*2][0], color=tableau20[i*2], lw=1, label=exp_path[i*2].split('_')[-1])
    #print("Mean reward for ", i, ": ", all_exp_data[i][0][-1], " - at iter 625: ", all_exp_data[i][0][624])
    print(exp_path[i].split('_')[-1], f' && {all_exp_data[i][0][311]:.2f} & ({all_exp_data[i][1][311]:.2f}) && {all_exp_data[i][0][624]:.2f} & ({all_exp_data[i][1][624]:.2f}) && {all_exp_data[i][0][1249]:.2f} & ({all_exp_data[i][1][1249]:.2f})')
ax_arch.set_xlabel('timesteps', fontsize=14)
ax_arch.set_ylabel('Return per Episode', fontsize=14)
