'''
    This categorical scatter plot was 
    created by @myrthings 2/4/20
    
    https://github.com/myrthings/catscatter
    
    We adapted it in order to allow for custom ordering of column, row titles.
'''
...
def catscatter(df,
    ...
    if col_ordering:
        colx_codes = {}
        for i in range(0, len(col_ordering)):
            colx_codes[col_ordering[i]] = i
    else:
        colx_codes=dict(zip(df[colx].sort_values().unique(),range(len(df[colx].unique()))))
    if row_ordering:
        coly_codes = {}
        for i in range(0, len(row_ordering)):
            coly_codes[row_ordering[i]] = i
        coly_codes = row_ordering 
    else:
