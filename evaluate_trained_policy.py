...
for eval_results in all_rew:
    print('Mean for ', ray_results_dir.split('_')[-1], '/', exp_params[run_it].split('0000')[-1][0], f': {np.mean(eval_results):.2f}, std.dev.: {np.std(eval_results):.2f}, CoT: {np.mean(all_cot[run_it]):.2f}, Vel.: {np.mean(all_vel[run_it]):.2f}')
    run_it += 1
print('Overall Mean for ', ray_results_dir.split('_')[-1], f': {np.mean(all_rew):.2f}, std.dev.: {np.std(all_rew):.2f}; CoT: {np.mean(all_cot):.2f}; Vel.: {np.mean(all_vel):.2f}, {np.std(all_vel):.2f}')
print(ray_results_dir.split('_')[-1], f' && {np.mean(all_rew):.2f} & ({np.std(all_rew):.2f}) && {np.mean(all_vel):.2f} & ({np.std(all_vel):.2f}) & {np.mean(all_cot):.2f}')
